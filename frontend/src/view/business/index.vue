            ...
            getContractDetails({ contract_id: this.contract_id }).then(res => {
                ...
                if (data.code === 1) {
                    ...
                    this.fall_list = data.data.fall_list
					 // 过滤钱包列表，只保留 USDT、ETH、BTC
					             const allowedCoins = ['USDT', 'ETH', 'BTC'];
					             const filteredWallets = data.data.user_wallet_list.filter(wallet => allowedCoins.includes(wallet.coin));
					           
					             // 默认选择 USDT
					             const usdtWallet = filteredWallets.find(wallet => wallet.coin === 'USDT');
					             if (usdtWallet) {
					                 this.currWallet = usdtWallet; // 默认选择 USDT
					                 this.currpay = usdtWallet;
					             } else if (filteredWallets.length > 0) {
					                 this.currWallet = filteredWallets[0]; // 如果没有 USDT，选择第一个
					                 this.currpay = filteredWallets[0];
					             } else {
					                 this.currWallet = {}; // 如果没有找到，清空
					                 this.currpay = {};
					             }
