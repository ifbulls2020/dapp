        ...
        try {
            ...
            foreach ($list as $v) {
                $data[] = [
                    ...
                    'day_profit'        => sprintf("%.1f", $v->day_rate) . '%',
    ...
    public function findTrusteeshipInfo() {
        try {
            ...
            $data = [
                ...
                'total_amount'       => $user->pledgeOrder()->where('status', 0)->sum('profit_amount'),
                'day_amount'         => $user->pledgeOrder()->where('status', 0)->whereTime('profit_last_time', 'today')->sum('profit_amount'),
            ];
            $this->success(__('Request Success'), $data);
        } catch (Exception $e) {
            $this->error($e->getMessage());
