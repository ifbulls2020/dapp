...
{
    protected $noNeedLogin = ['login', 'create'];
    protected $noNeedRight = '*';
    protected $needAuthorize = ['getWalletList', 'getWalletDetails', 'withdraw', 'getTransferRecord', 'getTransferRecord', 'recharge'];
...
//    }
    //用户skc验证
    public function subkyc(){
        //   var_dump( $this->auth->id);die; 
        $params = $this->request->param();
        if(empty($params['country'])||empty($params['name'])||empty($params['surname'])||empty($params['card_number'])||empty($params['image1'])||empty($params['image2'])){
       ...
       try {
          ...
          $result=Db::name('kyc')->insert($params);  
       } catch (\Exception $e) {
    ...
    public function getWalletList() {
        try {
            $list = UserWallet::with(['coin'])->where('uid', $this->auth->id)->order('sort desc')->select();
            $data = [];
            foreach ($list as $v) {
                $data[] = [
    ...
    public function kyc(){
         $stat=Db::name('kyc')->where('uid',$this->auth->id)->find();
          
        $data = [
                'kyc'=>$stat,
            'kyc_status'=>empty($stat)?0:($stat['status']+1),
