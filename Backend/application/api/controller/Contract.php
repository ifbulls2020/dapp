...
class Contract extends Api
{
    ...
    protected $needAuthorize = ['contractPlaceOrder'];
    ...
    public function getContractDetails() {
        try {
            ...
            $userWallet = UserWallet::alias('u')
                ...
                ->where('u.uid', $this->auth->id)->where("w.coin='BTC' OR w.coin='ETH' OR w.coin='USDT'")
    ...
    public function getKlineData() {
        try {
            ...
            $url = "https://api.huobi.pro/market/history/kline?period=" . $period . "&size=500&symbol=" . strtolower($contractConfig->coin . 'usdt');
